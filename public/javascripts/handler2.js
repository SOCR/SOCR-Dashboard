var tiles=[];
var listtemp={};
minCohort=0, maxCohort=0, numCohorts=0, cohortWidth=0;
cohortName='';
people=[];
var counter=0;
...
function populateDropdown()
{
    ...
    $.each(sourceDropDown, function(){
        ...
        .html(this.name)
        .appendTo("#datasource");
        listtemp[this.value] = this.subitems;
    });
    
    $("#datasource").change(function(){
        var value = $(this).val();
        var menu = $("#datavar");
        console.log(listtemp[value]);
        menu.empty();
        $.each(listtemp[value], function(){
            $("<option />")
            .attr("value", this.value)
            .html(this.name)
            .appendTo(menu);
...
$('#startdashboard').click(function(){
	var finished=true;
	$('#cohortselector input').each(function(){
		if($(this).val()=='')
			finished=false;
	})
	if(finished)
